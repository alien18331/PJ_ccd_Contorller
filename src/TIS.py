...
cap_Format = config['Camera']['format']
cap_FPS = config['Camera']['FPS']
cap_res_w = config['Camera']['resolution_w']
cap_res_h = config['Camera']['resolution_h']
...
class TIS:
    ...
    def selectDevice(self):
        ...
        if i > 0:
            ...
            print("Select Model: {0}\n".format(cfg_model))
            self.serialnumber = serials[0]
    ...
    def selectFormat(self):
        ...
        # ~ print("formats: {0}".format(formats))
        i = 0
        selFlg = 0
        f =[] 
        
        ''' set image format '''
        print("image format list:")
        for key,value in formats.items():
            if(key == cap_Format): cfg_key = key; cfg_key_index = i; selFlg = 1
            f.append(key)
            i = i + 1
            print("{}: {}".format(i, key))
        
        if not(selFlg):
            print("Invalid format: {}".format(cap_Format))
            cfg_key = 0
            cfg_key_index = 0
        
        # ~ print("Select Format: {0}\n".format(cfg_key))
        print("")
        format = f[cfg_key_index] 
        
        ''' set resolution '''        
        i = 0
        selFlg = 0
        print("image resolution list:")
        for res in formats[format].res_list:
            if(res.width==int(cap_res_w) and res.height==int(cap_res_h)): 
                res_index = i
                selFlg = 1
                # ~ print("resolution: {0}x{1}".format(res.width, res.height))
        ...
        if not(selFlg):
            print("Invalid resolution: {}x{}".format(cap_res_w, cap_res_h))
            res_index = 0
        
        # ~ res_index = 1 # select resolution: 640x480    
        # ~ print("Select resolution: {0}x{1}\n".format(formats[format].res_list[res_index].width, formats[format].res_list[res_index].height))
        print("")
        width=formats[format].res_list[res_index].width
        height =formats[format].res_list[res_index].height
        
        
        ''' set fps '''        
        o = 0
        selFlg = 0
        cfg_cap_fps = "{}/1".format(cap_FPS)
        print("camera FPS list:")
        for rate in formats[format].res_list[res_index].fps :
            if(rate == cfg_cap_fps): 
                cfg_fps = rate; cfg_fps_index = o; selFlg = 1
        ...
        if not(selFlg):
            print("Invalid FPS: {}".format(cap_FPS))
            # ~ cfg_fps = "30/1"
            cfg_fps_index = 0
        
        # ~ print("Select FPS: {0}\n".format(cfg_fps))
        framerate = formats[format].res_list[res_index].fps[cfg_fps_index] 
        
        print("\n===========================")
        print("setting camera config..\nimage format: {0}\nresolution: {1}x{2}\nFPS: {3}\n".format(format,width,height,framerate ))
